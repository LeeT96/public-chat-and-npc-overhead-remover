# Public Chat And NPC Overhead Chat Remover
Removes Selected Automatic Public Chat Entries and NPC Overhead Chat Messages
<br>
Took some code from "**Chop Chop Stop**" after installing it and realising it no longer worked.
<br>
<br>
This plugin currently works based on a selection of NPCs using checkboxes; however, I am currently working on allowing a list of NPC IDs to be provided instead so that any new, or missed, NPCs can be added without needing the plugin to be updated.

# Current Special Attacks Overhead Messages
- Dragon/Infernal/Crystal Pickaxe
- Dragon/Infernal/Crystal Axe
- Dragon/Infernal/Crystal Harpoon

# Current Special Attacks Public Chat Messages
- Dragon/Infernal/Crystal Pickaxe
- Dragon/Infernal/Crystal Axe
- Dragon/Infernal/Crystal Harpoon

# Current NPC Overhead Messages
* Cerberus
  - Summoned Souls
* Barbarians
* Nex
* God Wars Bosses
  - General Graardor
  - K'ril Tsutsaroth
  - Kree'arra
  - Commander Zilyana
* Deranged Archaeologist
* Crazy Archaeologist
* Chaos Fanatic
* Rogues (at the Rogue's Castle in the Wilderness)
* Town Crier
* Ramocean
* Prospector Percy

**_There are plans to allow specific Overhead messages to be allowed when muting an NPC, such as "Learn to read!". However, this must first be tested and be approved to be in accordance to Jagex's rules._**

**_There are plans to include NPC public chat messages in the future_**

# Current Other Overhead Messages
* Cuppa tea
* Come here toady

# Current Other Public Chat Messages
* Cuppa tea
* Come here toady
<br>
<br>

_Credits to "Chop Chop Stop" for the inspiration_
